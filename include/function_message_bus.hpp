#ifndef __FUNCTION_MESSAGE_BUS_HPP__
#define __FUNCTION_MESSAGE_BUS_HPP__
...
{
public:
	template<typename Function, typename = typename std::enable_if<!std::is_member_function_pointer<Function>::value>::type>
...
private:
	...
	{
		auto args_tuple = get_args_tuple(std::integral_constant<bool, std::is_same<U, FnKey>::value>{}, std::forward<U>(u), std::forward<Args>(args)...);
	...
	{
		auto it = invokers_.find(key);
		assert(it == invokers_.end(), "duplicate register");
	}

	template<typename Function>
	std::string get_key(std::string const & additional)
	{
		if (!additional.empty()) return additional;

		using bare_tuple_type = typename function_traits<Function>::bare_tuple_type;
		auto key = get_name_from_tuple(bare_tuple_type{});
		return key;
	}
	...
	struct invoker
	{
		...
		template<typename F, typename ... Args>
		static typename std::enable_if<!std::is_void<typename std::result_of<F(Args...)>::type>::value>::type call(const F& f, const std::tuple<Args...>& tp, void* result)
		{
...
#endif//__FUNCTION_MESSAGE_BUS_HPP__
